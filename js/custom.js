...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const inewcat = document.querySelector('.card-header__swiper');
    if (inewcat) {
        let ibannerSlider = new Swiper('.card-header__swiper', {
            slidesPerView: 1,
            effect: 'slide',
            loop: true,
            navigation: {
    ...
    const inewact = document.querySelector('.iactions__bottom_swiper');
    if (inewcat) {
        let iactionSlider = new Swiper('.iactions__bottom_swiper', {
            slidesPerView: 1,
            effect: 'slide',
            loop: true,
            navigation: {
                nextEl: '.swiper-button-act',
                prevEl: '.swiper-button-prev-act',
            },
        });
    }
    let sliderScrolls = document.querySelectorAll('.icats__bottom');
    let isDown = false;
    let startX;
    let scrollLeft;
    sliderScrolls.forEach(function (sliderScroll) {
